#!/usr/bin/env python3
"""
Test script for run_on_stream functionality.

Creates a mock PlusScriptJob, saves it to DynamoDB, launches Fargate,
waits for completion, and cleans up.

Usage:
    python test_scripts/test_run_stream.py [stream_key]

    Default stream_key: test_stream
"""
import boto3
import json
import os
...
def create_mock_job(hostname, username, stream_key):
    """Create a mock PlusScriptJob for testing run_on_stream."""
    job_uuid = str(uuid.uuid4())
    job_object_id = f'{hostname}/{username}/job.{job_uuid}'
    owner = f'{hostname}/{username}/stream.{stream_key}'
    now_ms = int(time.time() * 1000)

    job_dict = {
        'object_id': job_object_id,
        'owner': owner,
        'label': f'Test: Run on stream {stream_key}',
        'username': username,
...
def main():
    # Get stream_key from command line or use default
    stream_key = sys.argv[1] if len(sys.argv) > 1 else 'test_stream'

    print("=" * 60)
    print("TEST: run_on_stream via Fargate")
    print("=" * 60)
    ...
    job_object_id, job_dict = create_mock_job(hostname, username, stream_key)
